"""Shared pytest fixtures for cert-piper tests.

All certificate generation uses the cryptography library so that tests work
without embedding large PEM blobs in source files.
"""
...
def _build_der(key, subject_name, **kwargs):
    """Build and return raw DER bytes for a certificate."""
    pem = _build_cert(key, subject_name, **kwargs)
...
@pytest.fixture(scope="session")
def leaf_der(leaf_pem):
    """Raw DER bytes for the leaf certificate."""
    cert_obj = x509.load_pem_x509_certificate(leaf_pem.encode(), default_backend())
    return cert_obj.public_bytes(serialization.Encoding.DER)


@pytest.fixture(scope="session")
def leaf_b64_pem(leaf_pem):
    """Base64-encoded leaf PEM (the entire PEM text is base64-encoded)."""
    return base64.b64encode(leaf_pem.encode()).decode()


@pytest.fixture(scope="session")
def leaf_b64_der(leaf_der):
    """Base64-encoded raw DER certificate."""
    return base64.b64encode(leaf_der).decode()


@pytest.fixture(scope="session")
def bundle_pem(leaf_pem, ca_pem):
    """PEM bundle with two certificates."""
    return leaf_pem + "\n" + ca_pem


@pytest.fixture(scope="session")
def bundle_b64(bundle_pem):
    """Base64-encoded PEM bundle with two certificates."""
    return base64.b64encode(bundle_pem.encode()).decode()
